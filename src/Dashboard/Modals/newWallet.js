...
const NewWalletModal = ({props,visible, setVisible ,setModalVisible}) => {
  ...
  />
    ...
    <Text style={style.welcomeText}>In the next page , you will see your secret phrase</Text>
    <View style={{display:'flex', flexDirection:'row', marginTop:hp(5)}}>
    <Text style={style.welcomeText2}>If i loose my private key , my funds will be lost</Text>
...
<Switch value={Checked} onValueChange={()=>setCheckBox(!Checked)} />
        ...
        <View style={{display:'flex', flexDirection:'row', marginTop:hp(5)}}>
    <Text style={style.welcomeText2}>If i share my private key , my funds can get stolen</Text>
    <View style={{marginLeft:10}}>

<Switch value={Checked2} onValueChange={()=>setCheckBox2(!Checked2)} />
</View>
    ...
    <Button title={'Continue'} color={'green'} disabled={Checked&&Checked2?false:true} onPress={()=>{
        setLoading(true)
        setTimeout(() => {
            
            dispatch(Generate_Wallet2())
            .then((response)=>{
                if(response){
    ...
    <NewWalletPrivateKey Wallet={Wallet} SetVisible={setNewWalletPrivateKey} Visible={newWalletPrivateKey} setModalVisible={setModalVisible} setNewWalletVisible={setVisible}/>
    </Modal>
        </Animated.View>
  )
}
