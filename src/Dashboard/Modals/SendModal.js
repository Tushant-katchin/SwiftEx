...
import { SendTransaction, getNonce, SaveTransaction, decryptFile } from '../../utilities/utilities';
    ...
    const Balance = async () => {
      ...
      if(Type){
        if(JSON.parse(Type)=="Ethereum"){
          const response = await dispatch(getEthBalance(state.wallet.address?state.wallet.address:address))
          .then((res)=>{
          ...
          const response = await dispatch(getMaticBalance(state.wallet.address?state.wallet.address:address))
          .then((res)=>{
          ...
          const response = await dispatch(getXrpBalance(JSON.parse(wallet).classicAddress))
          .then((res)=>{
