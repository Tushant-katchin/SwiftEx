...
import { setPlatform, setWalletType } from '../../components/Redux/actions/auth';
import Modal from "react-native-modal";
import AsyncStorageLib from '@react-native-async-storage/async-storage';
import { useNavigation } from '@react-navigation/native';
import { decodeUserToken } from '../Auth/jwtHandler';
const PinViewModal = ({pinViewVisible,setPinViewVisible,setLoginVisible,username}) => {
            ...
            onButtonPress={async key => {
              ...
              if (key === "custom_right") {
                 if(status==='pinset'){
                  ...
                  const wallets = await  AsyncStorage.getItem(`${username}-wallets`)
                  ...
                  if(JSON.parse(Pin)===enteredPin){
                    ...
                    await AsyncStorageLib.getItem(`${username}-token`)
                    .then(async (token)=>{
                        ...
                        if(valid!==null){
                          ...
                          console.log(JSON.parse(wallets)[0])
                            ...
                            await AsyncStorageLib.setItem("currentWallet",username)
                            dispatch(setToken(token))
                            dispatch(setUser(username))
                            dispatch(setWalletType(JSON.parse(wallets)[0].walletType))
                            dispatch(setCurrentWallet(JSON.parse(wallets)[0].address,JSON.parse(wallets)[0].name,JSON.parse(wallets).privateKey))
