...
const getETHtoTokenPrice = async (tokenaddress,amount)=>{
   ...
   console.log(`Mid Price WETH --> ${token.address}:`, route.midPrice.toSignificant(6));
   console.log(`Mid Price ${token.address}: --> WETH`, route2.midPrice.toSignificant(6))
   ...
   return {token1totoken2:route.midPrice.toSignificant(6),
    token2totoken1:route2.midPrice.toSignificant(6),trade:tradeDetails
  }
   ...
   const tradeDetails ={
    slippageTolerance:slippageTolerance.toSignificant(1),
    minimumAmountOut:trade2.minimumAmountOut(slippageTolerance).toSignificant(6)
  }
  
   return {token1totoken2:route2.midPrice.toSignificant(6),
    token2totoken1:route.midPrice.toSignificant(6),trade:tradeDetails
  }
    ...
    const swapTokensforEth = async (privateKey,address,tokenaddress,amount) =>{
      try{
       ...
       const UNISWAP_ROUTER_ABI = [
        ...
        console.log(`Mid Price ${token.address}  --> WETH:`, route.midPrice.toSignificant(6));
        const slippageTolerance = new Percent('1', '100');
        ...
        if(trade.minimumAmountOut(slippageTolerance)){
          ...
          const uniswap = new ethers.Contract(
            ...
            const tx = await uniswap.swapExactTokensForETH(value,amountOut,
              path,
              to,
              deadline,
              {  gasLimit:500000}
