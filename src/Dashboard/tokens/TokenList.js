...
import { StyleSheet, Text, View, Button, ActivityIndicator, TouchableOpacity, ScrollView, FlatList} from 'react-native';
...
const TokenList = ({setVisible, setCoin0, setCoin1,data, coinType, walletType,setSwapType}) => {
  ...
  const renderHeader = () => (
      <Text style={style.title}>RN News</Text>
  )
...
<Card style={{width:wp(100), height:hp(10), backgroundColor:'white', borderRadius:10,marginLeft:wp(20),display:'flex'}}>
  ...
  return (
    ...
    >
     <TokenHeader setVisible={setVisible} name={name}/>   
      ...
      <SearchComponent clampedScroll={clampedScroll} data={data} setSearch={setSearch} setData={setData}/>

      {!Data?<View style={style.loading}>
        ...
        {loading===true?<View style={{marginBottom:wp(6),position:'absolute'}}><ActivityIndicator size='large' color={'black'} /></View>:<View></View>}
