...
import { Generate_Wallet, importAllWallets, AddToAllWallets, getDirectoryUri } from "../components/Redux/actions/auth";
 ...
 const CreateWallet=()=>{
    ...
    return response
      ...
      const getWallets = async ()=>{
        ...
        let data = JSON.parse(oldWallets)?JSON.parse(oldWallets):await state.wallets//await state.wallets?await state.wallets:oldWallets//AsyncStorageLib.getItem(`${user}-wallets`)
  ...
  const generate =   async () => {
   ...
   console.log(resp)
    setLoading(false)
       if(resp==400){
        return alert('account with the same name already exists. Please use a different name instead')
       }
       else{
       ...
       console.log(Folderuri)
           dispatch(Generate_Wallet(name,password,accName, dispatch, getDirectoryUri, Folderuri?Folderuri:''))
         .then(async (response) => {
